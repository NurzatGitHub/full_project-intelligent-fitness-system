...
class WorkoutSessionCreateSerializer(serializers.Serializer):
    ...
    notes = serializers.CharField(required=False, allow_blank=True, default="")
    client_uuid = serializers.CharField(required=False, allow_blank=True, default="", max_length=64)
    exercises = WorkoutExerciseWriteSerializer(many=True, required=False)

    def validate(self, attrs):
        started = attrs.get("started_at")
        finished = attrs.get("finished_at")
        if started and finished and finished < started:
            raise serializers.ValidationError(
                {"finished_at": "finished_at must be greater than or equal to started_at"}
            )
        return attrs
